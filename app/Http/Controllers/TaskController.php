    ...
    public function addTask(Request $request, TaskGroup $taskGroup): JsonResponse
    {
        ...
        if ($valid) {
            try {
                ...
                return response()->json(['message' => 'Successfully created new task', 'task' => ['id' => $task->id, 'isDone' => $task->isDone, 'content' => $task->content]], 200);
            } catch (\Exception $e) {
    ...
    public function updateTask(Request $request, TaskGroup $taskGroup, Task $task): JsonResponse
    {
        ...
        if ($valid) {
            try {
                ...
                return response()->json(['message' => 'Task has been successfully updated', 'task' => ['id' => $task->id, 'isDone' => $task->isDone, 'content' => $task->content]], 200);
            } catch (\Exception $e) {
